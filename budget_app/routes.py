...
def update_budget_category(budget_id, category_id):
    ...
    if form.validate_on_submit():
        if (
                ...
                and budget_category.used_amount == budget_category.threshold or form.used_amount.data > budget_category.available_amount
            ):
            flash(f"The category is already at its limit!", "danger")
            return redirect(url_for("update_budget_category",budget_id=budget_id,category_id=category_id,))
